...
def main():
    ...
    # corners = np.array([[-100,-100],[100,100]]) array con valores positivos y negativos
    wz = pointsGrid(corners)
    vistform(T, wz, 'Reescalar Coordenadas')
...
#%% EJERCICIO 3 -- DEFORMACIÓN DE CIZALLA
...
def main():
    ...
    b=2
    # generar el tipo de transformacion dando valores a la matriz T
    T = np.array([[a,0],[0,b]]) + np.array([[c,0],[0,d]]) @ np.array([[0,b],[a,0]])
    corners = np.array([[0,0],[100,100]])
    # corners = np.array([[-100,-100],[100,100]]) array con valores positivos y negativos
    wz = pointsGrid(corners)
    vistform(T, wz, 'Deformación de Cizalla de Coordenadas')
